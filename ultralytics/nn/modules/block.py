...
class C3k2Ghost(C2fGhost):
    ...
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        ...
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(
            C3kGhost(self.c, self.c, 2, shortcut, g) if c3k else GhostBottleneck(self.c, self.c, shortcut, g) for _ in range(n)
        )
...
class C3kGhost(C3Ghost):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5, k=3):
        ...
        c_ = int(c2 * e)  # hidden channels
        # self.m = nn.Sequential(*(RepBottleneck(c_, c_, shortcut, g, k=(k, k), e=1.0) for _ in range(n)))
        self.m = nn.Sequential(*(GhostBottleneck(c_, c_, shortcut, g, k=(k, k), e=1.0) for _ in range(n)))
